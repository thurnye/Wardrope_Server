...
function response(forAt: string): DressMeResponseDto {
  return {
    ...
    engine: 'baseline',
    weather: null,
    warnings: ['weather-not-requested'],
    recommendations: [{
...
describe('Dress Me API', () => {
  ...
  it('accepts only structured request-time context', async () => {
  ...
  it('rejects caller ownership, prompt, weather, provenance, and extra location fields', async () => {
